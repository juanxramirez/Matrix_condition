...
Input data are available within each folder of this repository, except for the habitat suitability models, which were derived from another study and are available only upon request (see https://globalmammal.org/habitat-suitability-models-for-terrestrial-mammals/).
...
We have tested our code using a desktop PC with a 3.6 GHz CPU and 16 GB of RAM.
...
Shows the distribution of the matrix condition in low-risk and high-risk species both globally (Supplementary Fig. 4) and at the scale of individual biogeographic realms (Supplementary Fig. 5). Requires `data_high_medium_first_last.txt` as input data.
